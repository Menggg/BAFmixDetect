...
VERSION <- "1.1.0-baseR-FinalReport-VCF-callrateFix"
...
apply_snp_qc <- function(sample, marker, baf, abgeno, maf, options,
  ...
  # ---------------------------------------------------------------------------
  # Stage 1: define the marker universe for SAMPLE GENOTYPE CALL RATE.
  #
  # These filters do not depend on whether BAF is valid or on downstream
  # regression-quality metrics. NoCall markers therefore remain in the
  # denominator and can correctly reduce sample call rate.
  # ---------------------------------------------------------------------------
  ...
  if (isTRUE(opt_get(options, "autosome_only", FALSE))) {
    if (is.null(chrom)) stopf(
      "--autosome_only requires FinalReport chromosome column: %s",
      opt_get(options, "colchrom", "Chr")
    )
  ...
  # Invalid genotype encodings are excluded from the call-rate denominator.
  valid_genotype_code <- !is.na(abgeno) & abgeno %in% 0:3
  counts$N_invalid_genotype <- sum(active & !valid_genotype_code, na.rm = TRUE)

  genotype_universe <- active & valid_genotype_code

  counts$N_genotype_callrate_denominator <- sum(genotype_universe)
  counts$N_called_genotype <- sum(
    genotype_universe & abgeno %in% c(0L, 1L, 2L),
    na.rm = TRUE
  )
  counts$N_nocall <- sum(
    genotype_universe & abgeno == 3L,
    na.rm = TRUE
  )

  counts$CallRate_genotype <- if (counts$N_genotype_callrate_denominator > 0) {
    counts$N_called_genotype / counts$N_genotype_callrate_denominator
  } else {
    NA_real_
  }

  # Now remove invalid genotype encodings before regression QC.
  remove_step(!valid_genotype_code, "N_invalid_genotype_removed")

  # ---------------------------------------------------------------------------
  # Stage 2: regression-specific marker QC.
  #
  # These filters determine which markers can enter BAF regression, but they
  # must NOT redefine the sample genotype call rate computed above.
  # ---------------------------------------------------------------------------
  ...
  if (min_gencall > 0) {
    if (is.null(gencall)) stopf(
      "--min_gencall > 0 requires FinalReport column: %s",
      opt_get(options, "colgencall", "GenCall Score")
    )
    remove_step(
      is.na(gencall) | gencall < min_gencall,
      "N_low_gencall_removed"
    )
  } else counts$N_low_gencall_removed <- 0L

  min_gcscore <- num_option(options, "min_gcscore", 0, 0, Inf)
  if (min_gcscore > 0) {
    if (is.null(gcscore)) stopf(
      "--min_gcscore > 0 requires FinalReport column: %s",
      opt_get(options, "colgcscore", "GC Score")
    )
    remove_step(
      is.na(gcscore) | gcscore < min_gcscore,
      "N_low_gcscore_removed"
    )
  ...
  if (min_intensity > 0) {
    if (is.null(intensity)) {
      stopf("--min_intensity > 0 requires usable intensity columns")
    }
    remove_step(
      is.na(intensity) | !is.finite(intensity) | intensity < min_intensity,
      "N_low_intensity_removed"
    )
  } else counts$N_low_intensity_removed <- 0L

  counts$N_afterQC <- sum(active)

  # This remains a regression-set diagnostic. It may be near 1 because invalid
  # BAF / low-quality markers have already been removed. Do not interpret this
  # value as the sample genotype call rate.
  counts$CallRate_after_markerQC <- if (sum(active) > 0) {
    1 - mean(abgeno[active] == 3L)
  } else {
    NA_real_
  }
  counts$CallRate_regression_set <- counts$CallRate_after_markerQC

  counts$Nhom_afterQC <- sum(
    active & abgeno %in% c(0L, 2L),
    na.rm = TRUE
  )

  min_callrate <- num_option(options, "min_callrate", 0, 0, 1)

  # Sample-level call-rate filtering now uses the corrected genotype call rate.
  counts$sample_pass_callrate <-
    is.na(counts$CallRate_genotype) ||
    counts$CallRate_genotype >= min_callrate
...
read_finalreport_sample_data <- function(file, sample, options, maf_vec) {
  ...
  if (!nrow(dat)) {
    ...
    attr(out, "qc") <- data.frame(
                                  sample = sample,
                                  N_total = 0L,
                                  N_genotype_callrate_denominator = 0L,
                                  N_called_genotype = 0L,
                                  N_nocall = 0L,
                                  CallRate_genotype = NA_real_,
                                  N_afterQC = 0L,
                                  CallRate_after_markerQC = NA_real_,
                                  CallRate_regression_set = NA_real_,
                                  Nhom_afterQC = 0L,
                                  sample_pass_callrate = NA,
  ...
  q$qc$CallRate_after_markerQC <- if (sum(keep) > 0) {
    1 - mean(abgeno[keep] == 3L)
  } else {
    NA_real_
  }
  q$qc$CallRate_regression_set <- q$qc$CallRate_after_markerQC
  q$qc$Nhom_afterQC <- sum(
    keep & abgeno %in% c(0L, 2L),
    na.rm = TRUE
  )
  min_callrate <- num_option(options, "min_callrate", 0, 0, 1)
  q$qc$sample_pass_callrate <-
    is.na(q$qc$CallRate_genotype) ||
    q$qc$CallRate_genotype >= min_callrate
...
testsamplecontamination <- function(
  baf,
  abgeno,
  maf,
  subset = NULL,
  callrate_override = NULL,
  ...
) {
  ...
  callrate <- if (
    !is.null(callrate_override) &&
    length(callrate_override) == 1L &&
    is.finite(callrate_override)
  ) {
    as.numeric(callrate_override)
  } else {
    1 - mean(abgeno == 3L)
  }
...
estimate_one <- function(file, format, sample, options, maf_vec) {
  ...
  if (!is.null(qc) && min_callrate > 0 && !isTRUE(qc$sample_pass_callrate[[1L]])) {
    stopf(
      "Sample %s failed --min_callrate %.4f (observed genotype call rate %.4f)",
      sample,
      min_callrate,
      qc$CallRate_genotype[[1L]]
    )
  }

  callrate_for_summary <- if (
    !is.null(qc) &&
    "CallRate_genotype" %in% names(qc) &&
    length(qc$CallRate_genotype) &&
    is.finite(qc$CallRate_genotype[[1L]])
  ) {
    qc$CallRate_genotype[[1L]]
  } else {
    NULL
  ...
  reg <- testsamplecontamination(
    dd$BAF,
    dd$ABGENO,
    dd$MAF,
    callrate_override = callrate_for_summary
  )
...
pipeline_usage <- function() {
  ...
  cat("  --min_callrate NUM        Minimum sample genotype call rate before BAF/regression QC [default: 0.90]\n")
  ...
  cat("  estimate_summary.tsv column 'callrate' = sample genotype call rate computed before BAF/GenCall/GC/intensity QC.\n")
  cat("  qc_summary.tsv additionally reports CallRate_after_markerQC / CallRate_regression_set for the retained regression marker set.\n\n")
