...
read_finalreport_sample_data <- function(file, sample, options, maf_vec) {
  ...
  needed <- unique(c(colmarker, colbaf, colab1, colab2, if (!identical(colsample, "NONE")) colsample else character()))
  numeric_cols <- character()
  if (mode %in% c("xy_y", "xy_x")) { needed <- unique(c(needed, "X", "Y")); numeric_cols <- c(numeric_cols, "X", "Y") }
  if (mode %in% c("raw_y", "raw_x")) { needed <- unique(c(needed, "X Raw", "Y Raw")); numeric_cols <- c(numeric_cols, "X Raw", "Y Raw") }
  if (mode %in% c("", "original", "Original", "ORIGINAL", "none", "NONE")) numeric_cols <- c(numeric_cols, colbaf)

  min_gencall <- num_option(options, "min_gencall", 0, 0, Inf)
  min_gcscore <- num_option(options, "min_gcscore", 0, 0, Inf)
  min_intensity <- num_option(options, "min_intensity", 0, 0, Inf)
  if (min_gencall > 0) { z <- opt_get(options, "colgencall", "GenCall Score"); needed <- unique(c(needed, z)); numeric_cols <- c(numeric_cols, z) }
  if (min_gcscore > 0) { z <- opt_get(options, "colgcscore", "GC Score"); needed <- unique(c(needed, z)); numeric_cols <- c(numeric_cols, z) }
  if (min_intensity > 0) { needed <- unique(c(needed, "X", "Y")); numeric_cols <- c(numeric_cols, "X", "Y") }
  if (isTRUE(opt_get(options, "autosome_only", FALSE))) needed <- unique(c(needed, opt_get(options, "colchrom", "Chr")))

  dat <- read_finalreport_table(file, needed, sep, numeric_cols = unique(numeric_cols))
  if (!identical(colsample, "NONE") && opt_has_value(sample)) dat <- dat[as.character(dat[[colsample]]) == as.character(sample), , drop = FALSE]
  if (!nrow(dat)) {
  ...
  maf <- maf_vec[marker]
  gencall <- if (min_gencall > 0) suppressWarnings(as.numeric(dat[[opt_get(options, "colgencall", "GenCall Score")]])) else NULL
  gcscore <- if (min_gcscore > 0) suppressWarnings(as.numeric(dat[[opt_get(options, "colgcscore", "GC Score")]])) else NULL
  intensity <- if (min_intensity > 0) suppressWarnings(as.numeric(dat[["X"]]) + as.numeric(dat[["Y"]])) else NULL
  chrom <- if (isTRUE(opt_get(options, "autosome_only", FALSE))) dat[[opt_get(options, "colchrom", "Chr")]] else NULL

  q <- apply_snp_qc(sample, marker, baf, abgeno, maf, options, gencall, gcscore, intensity, chrom, cnv)
  ...
  out <- data.frame(sample = smp[keep], marker = marker[keep], BAF = baf[keep], ABGENO = abgeno[keep], MAF = as.numeric(maf[keep]), stringsAsFactors = FALSE)
  attr(out, "qc") <- q$qc
  out
}
